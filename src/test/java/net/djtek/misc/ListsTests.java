...
public class ListsTests {
    @Test
    public void getIntersectionTests(){
        // one empty list
        List<Integer> list1 = Arrays.asList(new Integer[]{1, 2, 3, 4, 5});
        List<Integer> list2 = Arrays.asList(new Integer[]{});
        assertEquals(0, Lists.getIntersection(list1, list2).size());
        assertEquals(0, Lists.getIntersection(list2, list1).size());

        // one matching element tests
        // beginning element match
        list2 = Arrays.asList(new Integer[]{1});
        List<Integer> result = Lists.getIntersection(list1, list2);
        assertEquals(1, result.size());
        assertEquals(true, result.contains(1));

        result = Lists.getIntersection(list2, list1);
        assertEquals(1, result.size());
        assertEquals(true, result.contains(1));

        // end element match
        list2 = Arrays.asList(new Integer[]{5});
        result = Lists.getIntersection(list1, list2);
        assertEquals(1, result.size());
